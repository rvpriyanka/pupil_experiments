function edf = myBlink_regressout(edf, blinksmp, plotme, addBackSlowDrift)
...
% build a saccsmp matrix
saccsmp = [];
for iSac=2:length(edf.saccades.startTime)-1;
    saccsmp = cat(1, saccsmp, [find(edf.gaze.time==edf.saccades.startTime(iSac)) find(edf.gaze.time==edf.saccades.endTime(iSac))]);
end
...
[b,a] = butter(2, 0.01 / edf.samplerate, 'high');
...
[b,a] = butter(2, 10 / edf.samplerate, 'low');
...
newFs = 10;
downsmp = resample(dat.bpfilt,newFs, edf.samplerate);

% also downsample the sample idx for blinks and saccades
newblinksmp = round(blinksmp * (newFs/edf.samplerate));
newsaccsmp  = round(saccsmp * (newFs/edf.samplerate));
...
blinkIRFup = resample(blinkIRF, edf.samplerate, newFs);

% convolve with timepoints of events in original data
samplelogical = zeros(length(dat.pupil), 1);
offset = round(blinksmp(:, 2) + imp(1)*edf.samplerate);
...
% SAME FOR SACCADES
% upsample to the sample rate of the data
saccIRFup = resample(saccIRF, edf.samplerate, newFs);

% convolve with timepoints of events in original data
samplelogical = zeros(length(dat.pupil), 1);
offset = round(saccsmp(:, 2) + imp(1)*edf.samplerate);
offset(offset<1) = []; % remove those that we cant catch so early
samplelogical(offset)   = 1; % put 1s at these events
