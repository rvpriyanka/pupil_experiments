function [edf, newblinksmp] = myBlink_interpolate(edf, plotme)
...
for iBlink=2:length(edf.blinks.startTime)-2; % ATTN hack
    idx = cat(1, idx, [find(dat.time==edf.blinks.startTime(iBlink)) find(dat.time==edf.blinks.endTime(iBlink))]);
end
blinksmp = idx;
